...
//		Registry registry = LocateRegistry.createRegistry(1099);
		// TODO: 记得写一段代码，防止host发生变化, 也就是说host是会变的, 不能hardcode
		//       main class of broker也需要实现, 里面主要创建registry
		Registry registry = LocateRegistry.getRegistry("localhost", 1099);
//		for (String name : registry.list()) {
//			System.out.println("name in the list = "+name);
//		}
		for (String name : registry.list()) {
			if (name.startsWith("qs-") && !name.equals("qs-RemoteBindService")) {
