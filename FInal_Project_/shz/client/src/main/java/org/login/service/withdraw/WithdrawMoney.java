package org.login.service.withdraw;
...
public class WithdrawMoney implements Services {
    private static RestTemplate restTemplate = new RestTemplate();
    private static final String API_BASE_URL = "http://localhost:";
    private static Scanner scanner = new Scanner(System.in);
    public static void withdrawMoney(ClientInfo user) {
        ...
        ResponseEntity<Ack> response = restTemplate.postForEntity(API_BASE_URL+ "8081/withdraw", request, Ack.class);
        Ack ack = response.getBody();
        Map<Double, Integer> balanceCredits = Account.getUserBalance(user);
        if (ack.getAck()) {
            PrintController.withdrawPage(true, balanceCredits);
        } else {
            PrintController.withdrawPage(false, balanceCredits);
        }
    }
}
