...
import service.bond.bondService;
        ...
        try {
            ...
            consumer.setMessageListener(new MessageListener() {
                ...
                public void onMessage(Message message) {
                    try {
                        ...
                        System.out.println("Bond service has received client info of " + request.getInfo().name);
                        ...
                        System.out.println("Bond service has sent the product");
                        message.acknowledge();
            ...
            requestConsumer.setMessageListener(new MessageListener() {
                ...
                public void onMessage(Message message) {
                    try {
                        ...
                        System.out.println("Bond service has received purchase info of user " + request.userId);
                        String result = "Failed purchase";

                        try (java.sql.Connection conn = DatabaseUtil.getConnection()) {
                            String sql = "INSERT INTO bond_purchases (purchase_id, user_id, purchase_time, investment) VALUES (?, ?, ?, ?)";
                        ...
                        System.out.println("Bond service has sent the purchase result");
                        message.acknowledge();
                    } catch (JMSException e) {
                        e.printStackTrace();
                    }
            ...
            failureConsumer.setMessageListener(new MessageListener() {
                ...
                public void onMessage(Message message) {
                    try {
                        PurchaseInfo request = (PurchaseInfo) ((ObjectMessage) message).getObject();
                        System.out.println("Bond service has received the failure handle request of user " + request.userId);

                        try (java.sql.Connection conn = DatabaseUtil.getConnection()) {
                            String sql = "DELETE FROM bond_purchases WHERE purchase_id = ?";
                            try (PreparedStatement stmt = conn.prepareStatement(sql)) {
                                stmt.setInt(1, request.purchaseId);
                                int affectedRows = stmt.executeUpdate();
                                if (affectedRows > 0) {
                        ...
                        System.out.println("Bond service has handled the failure");
                        message.acknowledge();
